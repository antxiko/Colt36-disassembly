...
Merece la pena entender por qué esa copia existe, porque no es solo «traer el mapa». El área de trabajo es un **tablero que el juego modifica mientras juegas**: los bandidos no son sprites, se dibujan con `POKE` dentro de él. La línea 700 pone las cuatro casillas del bandido (`POKEBA%,C%` y las tres siguientes) y la 720 las borra poniendo ceros, con `BA%` apuntando siempre dentro de 0xDA00.

Y hay uno que no se borra: la línea 740, la del bandido que dispara, lo pinta y de ahí se pasa a la 745, la 747 y la 749, que vuelve al bucle. Ninguna deshace el `POKE`. O sea que **cada bandido que llega a disparar se queda pintado en el tablero para siempre**. Por eso hay que restaurar el mapa limpio al empezar cada nivel: sin esa copia, los que dispararon irían acumulándose y reapareciendo por todas partes según pasa el scroll.
