...
De ahí en adelante ya no queda cinta: los 20 bytes de 0x9088 ponen VARTAB —el puntero donde el intérprete construye su tabla de variables— en 0x8F60 y saltan a la rutina que ejecuta programas. Antes de eso escriben cinco bytes sobre 0x8000, y no es un detalle menor: en la cinta la primera línea del juego viene numerada 65535, por encima del máximo que MSX-BASIC admite, y esos cinco bytes la renumeran como la 4. Quien extraiga el bloque `CM1` y lo cargue por su cuenta no se lleva el juego: se lleva un programa que se rompe en cuanto busca una línea hacia atrás.
